...
*/
	...
	if (__GETC__(life_coplevel) > 0)  then {
		player setObjectTextureGlobal [0, "textures\corporal_uniform.jpg"]; // this is for all cops whitelisted or public.
	};

	if (__GETC__(life_coplevel) > 2)  then {
		player setObjectTextureGlobal [0, "textures\corporal_uniform.jpg"]; // this is for all whitelisted cops above the rank of 2 on the database (these numbers can be adjusted as needed)
	};
	...
	if ((uniform player) == "U_B_CombatUniform_mcam_worn")  then {
		player setObjectTextureGlobal [0, "textures\corporal_uniform.jpg"];
	};
